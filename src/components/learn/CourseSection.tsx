...
import { CheckCircle2, Circle, Lock, ArrowRight, Trophy, ChevronDown, ChevronUp } from "lucide-react";
...
export function CourseSection({ courseId, courseSlug, title, color, modules, quizAttempts, locked = false, lockMessage, defaultExpanded = false }: CourseSectionProps) {
    const DEV_UNLOCK_ALL = true; // DEV TOGGLE: Set to false before pushing to production
                        ...
                        <div className="mx-2 md:mx-6 mt-[-20px] pt-[40px] p-6 md:p-8 rounded-b-[2rem] bg-white/[0.02] border border-white/5 shadow-inner backdrop-blur-sm relative">
                            ...
                            <div className="space-y-6 relative">
                                {modules.map((item, index) => {
                                    ...
                                    return (
                                        <div key={item.id} className="relative pl-12 md:pl-16 group">
                                            {/* Dot on Timeline */}
                                            <div
                                                className={`absolute left-[5px] top-4 w-3 h-3 rounded-full border-[3px] transition-all duration-500 z-10
                                            ...
                                            <div
                                                className={`
                                                    ...
                                                    ${status === 'locked' ? 'bg-black/20 border-white/5 opacity-50' : 'bg-white/5 border-white/10 hover:border-white/20 hover:bg-white/10 hover:shadow-xl hover:-translate-y-1'}
                                                `}
                                            >
                                                <div className="flex items-start justify-between w-full mb-3">
                                                    <h3 className={`font-bold text-lg md:text-xl tracking-tight transition-colors ${status === 'completed' ? 'text-white' : status === 'current' ? colorOutput : 'text-gray-500'}`}>
                                                        {status !== 'locked' ? (
                                                            <Link href={`/learn/${courseSlug}/${item.slug || item.id}`} className="hover:underline">
                                                                {item.title}
                                                            </Link>
                                                        ) : (
                                                            item.title
                                                    ...
                                                    <div className="flex items-center gap-3 shrink-0 ml-4">
                                                        ...
                                                        {status === 'current' && <ArrowRight className={`w-6 h-6 ${colorOutput} animate-pulse`} />}
                                ...
                                <div className="relative pl-12 md:pl-16 mt-8">
                                    <div className={`absolute left-[5px] top-4 w-3 h-3 rounded-full border-[3px] transition-all duration-500 z-10 
